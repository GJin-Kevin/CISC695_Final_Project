...
class Backtesting:
    ...
    def RSI_single_parameter(self, period: int = 14, notification = True) -> pd.DataFrame:
        ...
        # Define the up days.
        df['up_day'] = df['change_in_price'].transform(
            lambda x : np.where(x >= 0, x, 0)
        )

        # Define the down days.
        df['down_day'] = df['change_in_price'].transform(
    ...
    def _rsi_add_signal(self, df):
        ...
        for i in range(1, df.shape[0]):
            ...
            elif df.loc[i-1, 'trend_indicator'] == 'buy' and df.loc[i, 'trend_indicator'] == 'buy':
                df.loc[i, 'signal'] = 'hold'

            #// if indicator turns from buy to none, then hold.
            elif df.loc[i-1, 'trend_indicator'] == 'buy' and df.loc[i, 'trend_indicator'] == 'none':
                df.loc[i, 'signal'] = 'hold'

            #// if indicator turns from buy to sell, then see=ll.
            elif df.loc[i-1, 'trend_indicator'] == 'buy' and df.loc[i, 'trend_indicator'] == 'sell':
                df.loc[i, 'signal'] = 'sell'


            #// if indicator turns from sell to buy, then hold.
            elif df.loc[i-1, 'trend_indicator'] == 'sell' and df.loc[i, 'trend_indicator'] == 'buy':
                df.loc[i, 'signal'] = 'buy'

            #// if indicator turns from sell to none, then hold.
            elif df.loc[i-1, 'trend_indicator'] == 'sell' and df.loc[i, 'trend_indicator'] == 'none':
                df.loc[i, 'signal'] = 'none'

            #// if indicator turns from sell to sell, then see=ll.
            elif df.loc[i-1, 'trend_indicator'] == 'sell' and df.loc[i, 'trend_indicator'] == 'sell':
                df.loc[i, 'signal'] = 'none'
            ...
            elif df.loc[i-1, 'trend_indicator'] == 'none' and df.loc[i, 'trend_indicator'] == 'buy' and (df.loc[i - 1, 'signal'] == 'none' or df.loc[i - 1, 'signal'] == 'sell'):
                df.loc[i, 'signal'] = 'buy'
            ...
            #// if indicator turns from none to sell, then see=ll.
